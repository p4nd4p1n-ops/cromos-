#!/usr/bin/env python3
"""Buscar rookie draft 2024 vía buscador de COMC y ver sus cartas."""
...
# buscador de COMC
for q in ["Stephon Castle Prizm RC", "Zaccharie Risacher Prizm RC"]:
    url = "https://www.comc.com/Search.aspx?search=" + urllib.parse.quote(q)
    html = get(url)
    m = re.search(r"<title>([^<]+)</title>", html)
    print(f"=== {q} === tam={len(html)} title={m.group(1).strip() if m else '?'}")
    # links a cartas
    links = re.findall(r'href="(/Cards/Basketball/2024-25[^"]*)"', html)
    print("links 2024-25:", len(links))
    for l in sorted(set(links))[:10]:
        print("  ", l)
    # si no hay, ver cualquier link de carta
    if not links:
        links2 = re.findall(r'href="(/Cards/[^"]*)"', html)
        print("links carta generales:", len(links2))

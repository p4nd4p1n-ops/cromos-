#!/usr/bin/env python3
"""1) Fecha release 2026-27 Topps Chrome Basketball (DDG vía FS).
2) Probar GetHistoricalSalesInfo con sesión fantasma (evolución precios)."""
...
print("=== 1) FECHA RELEASE 2026-27 TOPPS CHROME ===")
q = "2026-27 Topps Chrome Basketball release date checklist"
try:
    ...
    for m in re.finditer(r'result__a[^>]*>(.*?)</a>.*?result__snippet[^>]*>(.*?)</a>', html, re.S):
        ...
        if found < 6:
            ...
            found += 1
    if not found:
        print("sin resultados parseables; raw len:", len(html))
except Exception as e:
    print("DDG error:", str(e)[:100])

print()
print("=== 2) HISTORIAL VENTAS (GetHistoricalSalesInfo) con sesión ===")
# probar variantes de productKey para Queen card 31038651
for pk in ["31038651 0 ", "263.1 0 ", "2631 0 "]:
    try:
        d = fs({"cmd": "request.post",
                "url": "https://www.comc.com/CardPopupService.asmx/GetHistoricalSalesInfo",
                "postData": json.dumps({"sourceElement": None, "productKey": pk, "itemID": 0, "returnUrl": None}),
                "headers": {"Content-Type": "application/json; charset=UTF-8",
                            "X-Requested-With": "XMLHttpRequest",
                            "Referer": "https://www.comc.com/Cards/Basketball/2025-26/Topps_Chrome_-_Base/2631/Derik_Queen/31038651"},
                "session": "ghost", "cookies": COOKIES}, timeout=60000)
        resp = d.get("solution", {}).get("response", "")
        print(f"[pk={pk}] len={len(resp)}", flush=True)
        if resp and "Just a moment" not in resp and len(resp) > 100:
            print("  ->", clean(resp)[:400], flush=True)
    except Exception as e:
        print(f"[pk={pk}] error: {str(e)[:120]}", flush=True)

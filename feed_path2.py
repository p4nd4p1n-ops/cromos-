#!/usr/bin/env python3
"""Feed del producto EXACTO — Path con la URL completa de la carta.
Debe listar las copias de ESA carta. 11/08/2026.
"""
import json, urllib.request, re, sys, html as htmllib
...
# Path exacto del producto (base de Daniels)
path = "Cards/Football/2024/Panini_Donruss_Optic_-_Base/248/Rated_Rookie_-_Jayden_Daniels/28874190"
url = "https://www.comc.com/SearchFeed.aspx?PageSize=100&Sort%3dr&Path=" + urllib.parse.quote(path, safe="/")
...
for it in items[:10]:
    ...
    print("---")
    print("TITLE:", t.group(1).strip()[:90] if t else "?")
    print("GUID:", guid.group(1)[:130] if guid else "?")
    if desc:
        dsc = htmllib.unescape(desc.group(1))
        dsc = re.sub(r"<[^>]+>", " | ", dsc)
        print("DESC:", re.sub(r"\s+", " ", dsc)[:220])

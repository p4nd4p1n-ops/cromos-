#!/usr/bin/env python3
"""muro_knueppel.py — extrae el muro actual de Kon Knueppel y lo guarda en JSON."""
...
out = {"url": URL, "muro": muro,
       "por_precio": {f"{p:.2f}": c for p, c in sorted(por_precio.items())},
       "total_copias": sum(m["copias"] for m in muro), "n_ofertas": len(muro)}
json.dump(out, open("/root/comc-data/muro-knueppel-2026-08-08.json", "w"), ensure_ascii=False, indent=1)
print("total copias:", out["total_copias"], "| ofertas:", out["n_ofertas"])
for p, c in out["por_precio"].items():
    print(f"  ${p} x {c}")
print("detalle:")
for m in muro:
    print(f"  {m['vendedor']:24s} ${m['precio']:.2f} x {m['copias']}")

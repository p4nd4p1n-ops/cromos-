#!/usr/bin/env python3
"""Feed con Path completo de la carta — busca la marca de subasta en los items.
11/08/2026.
"""
...
url = ("https://www.comc.com/SearchFeed.aspx?PageSize=100&Sort%3dr&Path="
       + "Cards%2fFootball%2f2024%2fPanini_Donruss_Optic_-_Base_-_Purple_Shock_Prizm%2f248%2fRated_Rookie_-_Jayden_Daniels")
...
for it in items[:8]:
    ...
    print("GUID:", guid.group(1)[:110] if guid else "?")
    if desc:
        dsc = htmllib.unescape(desc.group(1))
        dsc = re.sub(r"<[^>]+>", " | ", dsc)
        print("DESC:", re.sub(r"\s+", " ", dsc)[:250])

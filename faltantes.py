#!/usr/bin/env python3
"""Buscar cartas base de los comps faltantes — con reintentos y ambos formatos de URL."""
...
FALTAN = [
    ("Bennedict Mathurin", "2022-23", "Bennedict_Mathurin"),
    ("Paolo Banchero", "2022-23", "Paolo_Banchero"),
    ("Matas Buzelis", "2024-25", "Matas_Buzelis"),
    ("Jaylen Wells", "2024-25", "Jaylen_Wells"),
]

for nombre, anio, slug in FALTAN:
    print(f"### {nombre} ({anio})")
    url = f"https://www.comc.com/Cards/Basketball/{anio}/Panini_Prizm_-_Base,sh,i100,{slug}"
    html = get(url)
    # ambos formatos: /2541/ y /254/
    links = re.findall(r'href="(/Cards/Basketball/' + anio + r'/Panini_Prizm_-_Base/\d+1/' + slug + r'/\d+)"', html)
    links += re.findall(r'href="(/Cards/Basketball/' + anio + r'/Panini_Prizm_-_Base/\d+/' + slug + r'/\d+)"', html)
    if not links:
        print("  NO encontrada")
        continue
    for l in sorted(set(links))[:2]:
        print("  base:", l)
        r = scan(l)
        if r:
            print(f"    {r['title'][:60]} | min={r['min']} seg={r['seg']} copias={r['copias']} v7={r['ventas_7d']} vel={r['vel']} dias={r['dias_inv']} turn={r['turnover']}%")
    print()

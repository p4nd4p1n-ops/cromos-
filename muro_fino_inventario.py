#!/usr/bin/env python3
"""muro_fino_inventario.py — scan fino de las 8 cartas del INVENTARIO (10/08/2026, +Knueppel 12/08).
Mismo motor que muro_fino_11.py: sleeps largos anti-rate-limit + 2º pase de fallidas.
12/08: verifica BuyItNow del PRIMER escalón NO propio (L-022) — [BN]/[SUB]/[?] en el muro.
Guarda: /root/comc-data/muro-fino-inv-<timestamp>.json + muro-fino-inv-ultimo.json
"""
...
CARTAS = [
    ("Flagg Chrome TC25-251.1-B", "https://www.comc.com/Cards/Basketball/2025-26/Topps_Chrome_-_Base/2511/Cooper_Flagg/31038638"),
    ("Riley Chrome TC25-271.1-B", "https://www.comc.com/Cards/Basketball/2025-26/Topps_Chrome_-_Base/2711/Will_Riley/31038659"),
    ("Clayton Chrome TC25-268.1-B", "https://www.comc.com/Cards/Basketball/2025-26/Topps_Chrome_-_Base/2681/Walter_Clayton_Jr/31038656"),
    ("Bryant Chrome TC25-264-B", "https://www.comc.com/Cards/Basketball/2025-26/Topps_Chrome_-_Base/264/Carter_Bryant/31038652"),
    ("Castle Chrome TC25-228.1-B", "https://www.comc.com/Cards/Basketball/2025-26/Topps_Chrome_-_Base/2281/Stephon_Castle/31038614"),
    ("Harper Chrome TC25-252.1-B", "https://www.comc.com/Cards/Basketball/2025-26/Topps_Chrome_-_Base/2521/Dylan_Harper/31038639"),
    ("Knueppel Chrome TC25-254.1-B", "https://www.comc.com/Cards/Basketball/2025-26/Topps_Chrome_-_Base/2541/Kon_Knueppel/31038641"),
    ("Daniels Optic DOPT24-248-B (NFL)", "https://www.comc.com/Cards/Football/2024/Panini_Donruss_Optic_-_Base/248/Rated_Rookie_-_Jayden_Daniels/28874190"),
]
...
def es_buynow(vendedor, card_path):
    """Pide la pagina del item del vendedor y comprueba BuyItNow (L-022).
    True = buynow real · False = subasta/soldout · None = sin_html (no sabemos)"""
...
def escanear_carta(nombre, url):
    ...
    # 12/08: verificar BuyItNow del 1er escalón NO propio (L-022)
    primer = next((m for m in muro_items if m["owner"] != "pinchonauta"), None)
    bn = None
    if primer:
        cm = re.search(r"/Cards/(.+?)(?:/Graded|$)", url)
        if cm:
            time.sleep(random.uniform(15, 25))  # separar peticiones (recurso único)
...
def main():
    ...
    resultados = {"fecha": datetime.datetime.now().strftime("%Y%m%d-%H%M%S"), "tipo": "muro-fino-inventario", "cartas": []}
    fallidas = []
    ...
    out = f"{DATA_DIR}/muro-fino-inv-{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
    json.dump(resultados, open(out, "w"), ensure_ascii=False, indent=1)
    print("GUARDADO " + out, flush=True)
    json.dump(resultados, open(f"{DATA_DIR}/muro-fino-inv-ultimo.json", "w"), ensure_ascii=False, indent=1)

#!/usr/bin/env python3
"""diag_anomalia.py — inspecciona la fila de PRTX560 ($1.49) en el allsellers de Knueppel:
¿subasta, best offer, dañada, o precio real? Imprime HTML crudo de la fila + contexto."""
...
open("/tmp/knueppel.html", "w").write(html)
print("HTML guardado, len:", len(html))

# 1) Buscar menciones de subasta en toda la página
for kw in ["auction", "Auction", "subasta", "bid", "Best Offer", "best offer", "Make Offer", "OBO"]:
    idxs = [m.start() for m in re.finditer(re.escape(kw), html)]
    print(f"'{kw}': {len(idxs)} apariciones", idxs[:5])

# 2) Extraer el bloque de la tabla allsellers
m = re.search(r'class="allsellers".*?</table>', html, re.S)
if m:
    block = m.group(0)
    print("\nBloque allsellers len:", len(block))
    # imprimir el bloque completo (formateado) para ver las filas
    open("/tmp/allsellers.html", "w").write(block)
    # filas crudas
    filas = re.findall(r'<tr[^>]*>.*?</tr>', block, re.S)
    print("filas:", len(filas))
    for f in filas:
        if "PRTX560" in f or "1.49" in f:
            print("\n=== FILA PRTX560/1.49 (cruda) ===")
            print(f[:2000])
else:
    print("No se encontró tabla allsellers")

# 3) contexto alrededor de 1.49
for mm in re.finditer(r"1\.49", html):
    s = max(0, mm.start() - 300)
    print("\n=== CONTEXTO 1.49 ===")
    print(html[s:mm.start() + 300].replace("\n", " "))
    break

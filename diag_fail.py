#!/usr/bin/env python3
"""Diagnóstico: página de jugador que devuelve sin_path (Kalkbrenner)."""
...
for nombre, slug, pid in [("Ryan Kalkbrenner", "Ryan_Kalkbrenner", "c436885"),
                           ("Kam Jones", "Kam_Jones", "c469358"),
                           ("Yanic Konan-Niederhauser", "Yanic_Konan-Niederhauser", "c495120")]:
    url = f"https://www.comc.com/Players/Basketball/{slug}/{pid}/Cards/Basketball/2025-26/Topps_Chrome_-_Base,sh,_RC,i100"
    d = fs({"cmd": "request.get", "url": url, "maxTimeout": 90000, "session": "ghost", "cookies": COOKIES})
    html = d.get("solution", {}).get("response", "")
    m = re.search(r"<title>([^<]+)</title>", html)
    print(f"=== {nombre} === tam={len(html)} title={m.group(1).strip() if m else '?'}")
    pat = re.compile(r'href="(/Cards/Basketball/2025-26/Topps_Chrome[^"]*)"')
    links = pat.findall(html)
    print("links chrome:", len(links))

#!/usr/bin/env python3
"""muro_knueppel5.py — fetch fresco (sesión nueva, anti-caché) + parseo robusto por filas."""
...
open("/tmp/knueppel5.html", "w").write(html)
print("len:", len(html))

# parseo robusto por <tr>
bloque = html
...
for f in filas:
    ...
    seller = h.unescape(sm.group(1)).strip() if sm else "(all)"
...
print("filas con precio:", len(out))
print("\n=== PRIMEROS 12 ===")
for p, s, q in out[:12]:

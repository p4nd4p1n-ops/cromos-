#!/usr/bin/env python3
"""feed_carta_url.py — trae un SearchFeed de COMC vía FlareSolverr y muestra los items.
Uso: feed_carta_url.py <url>"""
...
def main():
    ...
    if "Just a moment" in hh or len(hh) < 1000:
        print("BLOQUEADO o vacío")
        return
    # guardar para debug
    open("/root/comc-data/feed-knueppel.html", "w").write(hh)
    # items del feed (SearchFeed texto): patrón típico
    items = re.findall(r'href="/Cards/[^"]+"[^>]*>\s*([^<]+)</a>.*?\$([\d,]+\.\d{2})', hh, re.S)
    if not items:
        # otro patrón: buscar precios
        precios = re.findall(r"\$([\d,]+\.\d{2})", hh)
        print("precios encontrados:", len(precios))
        print(precios[:50])
        return
    for t, p in items[:60]:
        print(f"  {t.strip()[:60]:62} ${p}")

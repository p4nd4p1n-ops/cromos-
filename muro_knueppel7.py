#!/usr/bin/env python3
"""muro_knueppel7.py — fetch fresco + tabla allsellers COMPLETA (parseo corregido, sin filas basura)."""
...
open("/tmp/knueppel7.html", "w").write(html)
print("len:", len(html))

# tabla allsellers: buscar el ancla "All Sellers" y extraer su <table>
...
print(f"\n=== ALLSELLERS ({len(out)} filas) ===")
for p, s, q in out:
    tag = "SUB" if q is None else f"{q}"
    print(f"  ${p:.2f} | {s:24s} | {tag}")

# resumen solo precios con qty real
agg = {}
for p, s, q in out:
    if q:
        agg[p] = agg.get(p, 0) + q

#!/usr/bin/env python3
"""Test 2: sesión FlareSolverr + cookies completas de Pin."""
...
try:
    d = fs({"cmd": "session.create", "session": "pincomc", "cookies": cookies})
    print("session.create:", d.get("status"), d.get("message", ""))
except Exception as e:
    print("session.create error:", str(e)[:200])

url = "https://www.comc.com/Cards/Basketball/2025-26/Topps_Chrome_-_Base/2631/Derik_Queen/31038651"
try:
    d = fs({"cmd": "request.get", "url": url, "maxTimeout": 60000, "session": "pincomc"})
    html = d.get("solution", {}).get("response", "")
    open("/tmp/queen_ses.html", "w").write(html)
    print("tam:", len(html))

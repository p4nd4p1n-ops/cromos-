#!/usr/bin/env python3
"""Depurar: regex playerId flexible + ver links Prizm de Clingan."""
...
# 1) find_player_id flexible: buscar el slug y capturar cID después
for nombre in ["Stephon Castle", "Alex Sarr", "Matas Buzelis", "Donovan Clingan"]:
    slug = nombre.replace(" ", "_")
    html = get("https://www.comc.com/Search.aspx?search=" + urllib.parse.quote(nombre))
    pat = re.compile(r"/Players/Basketball/" + slug + r"/c(\d+)")
    ids = pat.findall(html)
    print(f"{nombre}: cIDs encontrados = {sorted(set(ids))[:5]}")
    if not ids:
        # ver qué jugadores salen
        otros = re.findall(r"/Players/Basketball/([A-Za-z_]+)/c(\d+)", html)
        print("   relacionados:", otros[:8])
    print()

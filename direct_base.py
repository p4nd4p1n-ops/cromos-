#!/usr/bin/env python3
"""Probar URLs directas de la base Prizm (sin /Graded/)."""
...
urls = [
    "https://www.comc.com/Cards/Basketball/2021-22/Panini_Prizm_-_Base/3061/Jalen_Green/20184713",
    "https://www.comc.com/Cards/Basketball/2021-22/Panini_Prizm_-_Base/306/Jalen_Green/20184713",
]
for u in urls:
    html = get(u)
    m = re.search(r"<title>([^<]+)</title>", html)
    items = re.findall(r'id="hp(\d+)"[^>]*>\$([\d,]+\.\d{2})<', html)
    prices = sorted(float(p.replace(",", "")) for _, p in items)
    print("URL:", u)
    print("  title:", m.group(1).strip()[:75] if m else "?")
    print("  items:", len(prices), "| min:", f"${prices[0]:.2f}" if prices else "-")

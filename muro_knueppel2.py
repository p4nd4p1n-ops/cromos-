#!/usr/bin/env python3
"""muro_knueppel2.py — muro actual de Knueppel (07:53 UTC) tras el cambio de precios."""
...
open("/tmp/knueppel2.html", "w").write(html)
...
out = {"por_precio": {f"{p:.2f}": c for p, c in sorted(por_precio.items())},
       "muro": muro, "total_copias": sum(m["copias"] for m in muro), "n_ofertas": len(muro)}
json.dump(out, open("/root/comc-data/muro-knueppel-0753.json", "w"), ensure_ascii=False, indent=1)
print("total copias:", out["total_copias"], "| ofertas:", out["n_ofertas"])
for p, c in out["por_precio"].items():
    print(f"  ${p} x {c}")
print("detalle (sin qty = subasta):")
for m in muro:

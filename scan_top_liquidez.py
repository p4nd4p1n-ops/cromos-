#!/usr/bin/env python3
"""Escaneo de liquidez (vel/sem + gap) de las candidatas top del feed de jugadores.
Feed de 8 jugadores → filtrar (≤$5.05, ≥10 copias, sin gradadas/autos) → excluir conocidas
→ escanear las 8 con más copias. Espaciado 35-45s. 11/08/2026.
"""
...
JUGADORES = ["Cooper Flagg", "Dylan Harper", "Ace Bailey", "VJ Edgecombe",
             "Kon Knueppel", "Hugo Gonzalez", "Collin Murray-Boyles", "Nikola Jokic"]

# Códigos que ya tenemos (inventario + punto de mira + watch list) — no escanear
CONOCIDAS = ["Topps Chrome - [Base] #252.1", "Topps Chrome - [Base] #251.1",
             "Topps Chrome - [Base] #254.1", "Topps Chrome - [Base] #253.1",
             "Topps Chrome - [Base] #264", "Topps Chrome - [Base] #228.1",
             "Topps Chrome - [Base] #271.1", "Topps Chrome - [Base] #268.1",
             "Topps Chrome - [Base] #255.1", "Bowman University Chrome - [Base] #22",
             "Topps NBA Living Set #11", "Topps - [Base] #201", "Topps - [Base] #204"]
...
# 1) Recoger candidatas de todos los feeds
todas = {}
for jug in JUGADORES:
    ...
    for p, q, t, u in feed:
        ...
        if p > 5.05 or not q or q < 10:
            continue
        if any(x in tl for x in ['psa', 'cga', 'cgc', 'bgs', 'sgc', 'auto', 'relic',
                                 'gold', 'black', 'cosmic dust', 'gold foil']):
            continue
        if any(c in t for c in CONOCIDAS):
            continue
        # dedup por título
        todas[t] = (p, q, t, u)
    time.sleep(random.randint(30, 40))

print(f"Candidatas nuevas totales: {len(todas)}", flush=True)
# Ordenar por copias (las que más mercado tienen)
top = sorted(todas.values(), key=lambda x: x[1], reverse=True)[:8]
print("=== TOP 8 POR COPIAS A ESCANEAR ===", flush=True)
for p, q, t, u in top:
...
for r in sorted(resultados, key=lambda x: x.get("vel_sem", -1), reverse=True):
    ...
    print(f"  {ok} | {r['carta'][:42]:<44} vel/sem {r['vel_sem']:>3} ({r['dias']}d) | muro {r['min']}/{r['seg']} | gap {r['gap']}% | {r['copias']} copias", flush=True)

#!/usr/bin/env python3
"""muro_fino_11.py v2 — scan fino de las 11 cartas del Punto de mira.
v2: sleeps largos anti-rate-limit + 2º pase para las que fallan + guardado con
timestamp (para que muro_compare.py pueda comparar pasadas).
"""
import json, urllib.request, re, time, random, datetime, os, sys
...
CARTAS = [
    ...
    ("Kon Knueppel Chrome TC25-254.1-B", "https://www.comc.com/Cards/Basketball/2025-26/Topps_Chrome_-_Base/2541/Kon_Knueppel/31038641"),
    ...
    ("Harper Bowman BUC24-22-B", "https://www.comc.com/Cards/Basketball/2024-25/Bowman_University_Chrome_-_Base/22/Dylan_Harper/28629778"),
    ("VJ Edgecombe Topps T25-203-B", "https://www.comc.com/Cards/Basketball/2025-26/Topps_-_Base/203/VJ_Edgecombe/30579237"),
    ("Curry/LeBron/KD TNO24-26.1-B", "https://www.comc.com/Cards/Olympic/2024/Topps_Now_OlympicParalympic_Games_-_Base/261/Stephen_Curry_LeBron_James_Kevin_Durant_(Red_Topps_Logo)/26528513"),
    ("LeBron Topps T25-150-B", "https://www.comc.com/Cards/Basketball/2025-26/Topps_-_Base/150/LeBron_James/30579184"),
]

BLOCK_INDICATORS = ["access denied", "captcha", "please verify", "unusual traffic", "sign in to continue"]
DATA_DIR = "/root/comc-data"
OUT_PATTERN = f"{DATA_DIR}/muro-fino-%Y%m%d-%H%M%S.json"
...
def parse_muro(html):
    """Devuelve lista de items individuales (item_id, precio, owner)."""
...
def escanear_carta(nombre, url):
    ...
    return {"carta": nombre, "url": url,
            ...
            "muro": resumen, "muro_txt": muro_txt[:600], "muro_items": muro_items}
...
def main():
    ...
    resultados = {"fecha": datetime.datetime.now().strftime("%Y%m%d-%H%M%S"), "tipo": "muro-fino-11", "cartas": []}
    fallidas = []

    # 1er pase: sleeps largos entre cartas (25-45s) para no quemar FlareSolverr
    ...
    # 2º pase solo fallidas, tras pausa larga (rate-limit cooling)
    if fallidas:
    ...
    out = datetime.datetime.now().strftime(OUT_PATTERN)
    json.dump(resultados, open(out, "w"), ensure_ascii=False, indent=1)
    print("GUARDADO " + out, flush=True)
    # también copia fija "ultimo" para el comparador
    json.dump(resultados, open(f"{DATA_DIR}/muro-fino-ultimo.json", "w"), ensure_ascii=False, indent=1)

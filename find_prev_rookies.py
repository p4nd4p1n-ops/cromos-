#!/usr/bin/env python3
"""Buscar rookie temporada pasada (draft 2024) en COMC y extraer su historial de ventas."""
...
# 1) buscar página del jugador Stephon Castle en COMC (draft 2024, Spurs)
# probar IDs conocidos de rookies 2024-25
for nombre, pid in [("Stephon Castle", "c439593"), ("Zaccharie Risacher", "c437849")]:
    url = f"https://www.comc.com/Players/Basketball/{nombre.replace(' ', '_')}/{pid}"
    html = get(url)
    m = re.search(r"<title>([^<]+)</title>", html)
    print(f"=== {nombre} ({pid}) === tam={len(html)} title={m.group(1).strip() if m else '?'}")
    # buscar links a cartas de sets 2024-25
    links = re.findall(r'href="(/Cards/Basketball/2024-25/[^"]*)"', html)
    print("links 2024-25:", len(links))

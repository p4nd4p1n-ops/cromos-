#!/usr/bin/env python3
"""Escaneo de ÉLITE — liquidez en cartas premium (precio 5-50$) de jugadores top.
Objetivo: ver DÓNDE está la liquidez real del mercado (Pin: "si en elite hay, ahí vamos").
...
JUGADORES = ["Victor Wembanyama", "LeBron James", "Stephen Curry", "Kevin Durant",
             "Nikola Jokic", "Anthony Edwards", "Giannis Antetokounmpo", "Shai Gilgeous-Alexander"]
...
# 1) Feed de los 8 élite → candidatas premium (precio 5-50, copias ≥ 10)
todas = {}
for jug in JUGADORES:
    ...
    for p, q, t, u in feed:
        tl = t.lower()
        if not (5.0 <= p <= 50.0) or not q or q < 10:
            continue
        if any(x in tl for x in ['psa', 'cga', 'cgc', 'bgs', 'sgc', 'auto', 'relic',
                                 'patch', 'gold', 'black', 'cosmic dust']):
            continue
        todas[t] = (p, q, t, u)
    time.sleep(random.randint(30, 40))

print(f"Cartas élite (5-50$, ≥10 copias) totales: {len(todas)}", flush=True)
top = sorted(todas.values(), key=lambda x: x[1], reverse=True)[:8]
print("=== TOP 8 ÉLITE POR COPIAS A ESCANEAR ===", flush=True)
for p, q, t, u in top:
    print(f"  ${p:.2f} x{q} | {t[:65]}", flush=True)

# 2) Escanear liquidez
print("\n=== ESCANEANDO LIQUIDEZ ÉLITE ===", flush=True)
resultados = []

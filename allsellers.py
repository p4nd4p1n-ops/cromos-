#!/usr/bin/env python3
"""All Sellers por carta: vendedor -> precio -> copias (detectar muros de un solo vendedor)."""
...
for nombre, url in CARTAS:
    try:
        ...
        print(f"=== {nombre} ===")
        # filas allsellers: <td class="seller"><a ...>NOMBRE</a></td> <td class="displayprice">...$X.XX</td> <td class="qtyforsale">... (N)</td>
        filas = re.findall(
        ...
        for seller, precio, qty in filas:
            ...
            print(f"  {seller:28s} ${precio:>8s}  ({qty} copias)")

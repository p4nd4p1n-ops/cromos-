#!/usr/bin/env python3
"""Localizar carta base Prizm 2024-25 de Castle y Sarr + extraer Recent Sales."""
...
# buscar carta base Prizm RC
for q, jugador in [("Stephon Castle 2024-25 Prizm RC base", "Stephon Castle"),
                    ("Alex Sarr 2024-25 Prizm RC base", "Alex Sarr")]:
    html = get("https://www.comc.com/Search.aspx?search=" + urllib.parse.quote(q))
    # buscar el link de Panini Prizm - Base (sin variaciones)
    links = re.findall(r'href="(/Cards/Basketball/2024-25/Panini_Prizm(?:/[^"]*)?)"', html)
    base = [l for l in sorted(set(links)) if "Prizm_-_Base" in l or "/Base/" in l or "/Prizm/" in l]
    print(f"=== {jugador} ===")
    if not links:
        print("  sin links Panini_Prizm")
        continue
    # preferir el que tenga "Base" sin más sufijos
    candidatos = [l for l in sorted(set(links)) if l.count("/") == 5]
    for l in candidatos[:5]:
        print("  ", l)

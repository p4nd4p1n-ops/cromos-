#!/usr/bin/env python3
"""Feed del producto con Path 100% codificado (quote safe='').
Debe listar las copias de ESA carta con sus estados. 11/08/2026.
"""
import json, urllib.request, re, sys, html as htmllib, urllib.parse
...
url = ("https://www.comc.com/SearchFeed.aspx?PageSize=100&Sort%3dr&Path="
...
for it in items[:8]:
    ...
    print("GUID:", guid.group(1)[:140] if guid else "?")
    if desc:
        dsc = htmllib.unescape(desc.group(1))
        dsc = re.sub(r"<[^>]+>", " | ", dsc)
        print("DESC:", re.sub(r"\s+", " ", dsc)[:200])

#!/usr/bin/env python3
"""Test 4: limpiar sesiones, crear UNA sesión con cookies de Pin, fetch Queen."""
...
# 1) listar y destruir TODAS las sesiones
try:
    d = fs({"cmd": "sessions.list"}, timeout=30000)
    ses = d.get("sessions", [])
    print("sesiones activas:", ses)
    for s in ses:
        try:
            fs({"cmd": "sessions.destroy", "session": s}, timeout=30000)
            print("destruida:", s)
        except Exception as e:
            print("no destruida:", s, str(e)[:80])
except Exception as e:
    print("list error:", str(e)[:200])

time.sleep(2)

# 2) crear sesión limpia con cookies
try:
    d = fs({"cmd": "sessions.create", "session": "pincomc", "cookies": cookies}, timeout=60000)
...
# 3) fetch Queen con la sesión
url = "https://www.comc.com/Cards/Basketball/2025-26/Topps_Chrome_-_Base/2631/Derik_Queen/31038651"
try:
    d = fs({"cmd": "request.get", "url": url, "maxTimeout": 90000, "session": "pincomc"})
    html = d.get("solution", {}).get("response", "")
    open("/tmp/queen_ses3.html", "w").write(html)
    print("tam:", len(html))

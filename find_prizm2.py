#!/usr/bin/env python3
"""Buscar set Panini Prizm 2024-25 de Castle — URLs exactas."""
...
# búsqueda más simple: "2024-25 Panini Prizm Stephon Castle"
for q in ["2024-25 Panini Prizm Stephon Castle", "2024-25 Panini Prizm Alex Sarr"]:
    html = get("https://www.comc.com/Search.aspx?search=" + urllib.parse.quote(q))
    m = re.search(r"<title>([^<]+)</title>", html)
    print(f"=== {q} === title={m.group(1).strip() if m else '?'}")
    links = re.findall(r'href="(/Cards/Basketball/2024-25/[^"]*)"', html)
    prizm = [l for l in sorted(set(links)) if "Prizm" in l and "Optic" not in l]
    print("links con Prizm:", len(prizm))
    for l in prizm[:12]:
        print("  ", l)
    print()

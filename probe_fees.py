#!/usr/bin/env python3
"""Buscar fees COMC: página oficial + resultados web."""
...
def get(url, cookies=None, session="ghost"):
    payload = {"cmd": "request.get", "url": url, "maxTimeout": 90000, "session": session}
    if cookies:
...
# 1) página de ayuda/fees de COMC (intentos)
for u in ["https://www.comc.com/Help/Fees",
          "https://www.comc.com/Help",
          "https://www.comc.com/Help/About_COMC"]:
    try:
        html = get(u, COOKIES)
        if "Just a moment" in html or len(html) < 1000:
            print(f"[{u}] bloqueado/pequeño ({len(html)})")
            continue
        txt = clean(html)
        # buscar párrafos con fee/commission/percent
        for m in re.finditer(r"[^.]*?(?:fee|commission|percent|%|credit)[^.]*\.", txt, re.I):
            s = m.group(0).strip()
...
# 2) búsqueda DDG vía FlareSolverr
q = "COMC seller fees commission percentage cashout"
try:
    ...
    for m in re.finditer(r'result__a[^>]*>(.*?)</a>.*?result__snippet[^>]*>(.*?)</a>', html, re.S):
        ...
        print(h.unescape(re.sub(r"\s+", " ", t))[:250])
except Exception as e:

#!/usr/bin/env python3
"""diag_prtx3.py — perfil del vendedor PRTX560: ¿tiene la copia de Knueppel en subasta?"""
...
url = "https://www.comc.com/Users/PRTX560/"
d = fs({"cmd": "request.get", "url": url, "maxTimeout": 90000, "session": "ghost", "cookies": COOKIES})
html = d.get("solution", {}).get("response", "")
open("/tmp/prtx560_profile.html", "w").write(html)
print("perfil len:", len(html))

for kw in ["auction", "Auction", "Bid", "bid", "Time Left", "time left", "Knueppel", "1.49"]:
    n = len(re.findall(re.escape(kw), html))
    print(f"  '{kw}': {n}")

# contexto de Knueppel si existe
for mm in list(re.finditer(r"Knueppel", html))[:3]:
    s = max(0, mm.start() - 250)
    e = min(len(html), mm.start() + 250)
    print("\nCTX Knueppel:", re.sub(r"\s+", " ", html[s:e]))

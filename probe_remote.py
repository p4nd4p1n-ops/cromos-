#!/usr/bin/env python3
"""probe_remote.py — comprueba si el HTML de una carta expone el aviso 'Item Stored Remotely / Standard Shipping Only' por item del muro."""
...
open("/root/comc-data/harper-page.html", "w").write(html)

print("=== patrones ===")
for pat in ["RemoteItems", "Stored Remotely", "Standard Shipping", "remotely", "Remote"]:
    print(pat, "->", len(re.findall(pat, html, re.IGNORECASE)))

print("=== contexto 'Stored Remotely' / 'Standard Shipping' ===")
for m in re.finditer(r".{250}(?:Stored Remotely|Standard Shipping).{150}", html, re.IGNORECASE | re.S):
    print(repr(m.group(0)))
    print("---")

print("=== items del muro (hp) y lo que hay alrededor ===")
for m in re.finditer(r'id="hp(\d+)"[^>]*>\$([\d,]+\.\d{2})<', html):
    iid = m.group(1)
    seg = html[m.start():m.start()+2200]
    # buscar en el segmento del item si hay marca remota/shipping
    remoto = "REMOTO" if re.search(r"Stored Remotely|Standard Shipping", seg, re.I) else "normal"
    print(f"item {iid} ${m.group(2)} -> {remoto}")

#!/usr/bin/env python3
"""Escaneo NFL + SOCCER (corregido: sin SportID, búsqueda global).
Pin 11/08/2026: curiosidad multi-deporte. Baseball ya escaneado (nada pasa).
Espaciado 35-45s.
"""
...
DEPORTES = {
    "NFL": ["Patrick Mahomes", "CJ Stroud", "Jayden Daniels", "Caleb Williams", "Travis Hunter",
            "Ashton Jeanty", "Shedeur Sanders", "Cam Ward", "Bijan Robinson", "Justin Jefferson"],
    "SOCCER": ["Lionel Messi", "Lamine Yamal", "Kylian Mbappe", "Jude Bellingham", "Erling Haaland",
               "Vinícius Junior", "Rodri", "Cole Palmer"],
}
...
def get_feed(jugador):
    url = ("https://www.comc.com/SearchFeed.aspx?Search="
...
todas = {}
for dep, jugadores in DEPORTES.items():
    ...
    print(f"  {dep}: {len(deps)} candidatas (≤$5.05, ≥10 copias)", flush=True)
...
print("\n=== TOP A ESCANEAR ===", flush=True)
for k, (p, q, t, u, dep) in sel.items():
    print(f"  [{dep}] ${p:.2f} x{q} | {t[:65]}", flush=True)
...
print("\n=== RESUMEN (M-010: vel/sem ≥ 13 + gap ≥ 5.3%) ===", flush=True)
for dep in DEPORTES:
    print(f"\n--- {dep} ---", flush=True)

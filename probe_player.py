#!/usr/bin/env python3
"""Probe: URL de jugador COMC (RC del set) — ver estructura y enlaces a cartas."""
...
url = "https://www.comc.com/Players/Basketball/Kon_Knueppel/c469745/Cards/Basketball/2025-26/Topps_Chrome_-_Base,sh,_RC,i100"
d = fs({"cmd": "request.get", "url": url, "maxTimeout": 90000, "session": "ghost", "cookies": COOKIES})
html = d.get("solution", {}).get("response", "")
open("/tmp/knueppel_player.html", "w").write(html)
print("tam:", len(html))
m = re.search(r"<title>([^<]+)</title>", html)
print("title:", m.group(1).strip() if m else "?")
pat = re.compile(r'href="(/Cards/Basketball/2025-26/Topps_Chrome_-_Base/[0-9]+[^"]*)"')
links = pat.findall(html)
print("links carta:", len(links))

#!/usr/bin/env python3
"""Mide liquidez REAL de sets Panini (Prizm/Optic/Select/Mosaic) — 11/08/2026.
Feed de Flagg → filtrar variantes de esos sets → escanear vel/sem + gap.
"""
...
# 1) Feed de Flagg — encontrar sus cartas en Prizm/Optic/Select/Mosaic
print("=== FEED FLAGG — buscando sets Panini ===")
feed = get_feed("Cooper Flagg")
panini = []
for p, q, t, u in feed:
    tl = t.lower()
    if any(s in tl for s in ["prizm", "optic", "select", "mosaic", "hoops"]) and "auto" not in tl and "relic" not in tl:
        panini.append((p, q, t, u))
print(f"Sets Panini de Flagg encontrados: {len(panini)}")
for p, q, t, u in panini[:15]:
    print(f"  ${p:.2f} x{q} | {t[:70]}")

# 2) Escanear las 3-4 más baratas (base de cada set) — espaciado
print("\n=== ESCANEANDO LIQUIDEZ (vel/sem + gap) ===")
a_escanear = [x for x in panini if x[0] <= 10][:4]
resultados = []
for i, (p, q, t, u) in enumerate(a_escanear):
    print(f"[{i+1}/{len(a_escanear)}] {t[:50]}...", flush=True)
    r = escanear(t[:60], u)
    resultados.append(r)
    print(f"    {r}", flush=True)
    if i < len(a_escanear) - 1:
...
for r in resultados:
    ...
    print(f"  {ok} | {r['carta'][:40]:<42} vel/sem {r['vel_sem']:>3} ({r['dias']}d) | muro {r['min']}/{r['seg']} | gap {r['gap']}% | {r['copias']} copias")
